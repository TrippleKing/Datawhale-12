...
$$
从而，得到$||w_3||=\frac{1}{2}||w_1^*||+\frac{1}{2}||w_2^*||=||\frac{w_1^*+w_2^*}{2}||$，当且仅当$w_1^*$与$w_2^*$两向量共线同向时取等，即：
$$
w_1^*=\lambda w_2^*
$$
又因为$||w_1^*||=||w_2^*||=min$，则有$||w_1^*||=|\lambda|\cdot||w_2^*||$，可以推出：
$$
|\lambda|=1\rightarrow\lambda=\pm1
$$
分两种情况，分别推导：

1. 若$\lambda=-1$，则$w_1^*=w_2^*=0$，代入约束条件则有$y_i(0\cdot x+b)-1\ge0,i=1,2,...,N$，显然$b$无解，该情况不可取。
2. 若$\lambda=1$，则$w_1^*=w_2^*$，可以用同一个符号来表示，记为$w^*$。

下面，再来证明$b_1^*=b_2^*$:

假设$x_1'$和$x_2'$是集合$\lbrace x_i|y_i=+1\rbrace$中分别对应于$(w^*,b_1^*)$和$(w^*,b_2^*)$使得优化问题不等式等号成立的点，$x_1''$和$x_2''$是集合$\lbrace x_i|y_1=-1\rbrace$中分别对应于$(w^*,b_1^*)$和$(w^*,b_2^*)$使得优化问题不等式等号成立的点，即：
$$
w^*\cdot x_1'+b_1^*=1
$$
...
$$
\rightarrow b_1^*-b_2^*=-\frac{1}{2}[w^*\cdot(x_1'-x_2')+w^*\cdot(x_1''-x_2'')]
$$

我们将$x_1'$和$x_2'$分别代入到$(w^*,b_2^*)$和$(w^*,b_1^*)$组成的超平面中，则有
$$
w^*\cdot x_2'+b_1^*\ge1=w^*\cdot x_1'+b_1^*
$$

$$
w^*\cdot x_1'+b_2^*\ge1=w^*\cdot x_2'+b_2^*
$$
